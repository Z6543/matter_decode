...
CHIP_ERROR PASESession::DeriveSecureSession(CryptoContext & session)
{
    ...
    // Log the HKDF key material for debugging
    // Note: This only works with RawKeySessionKeystore where keys are stored directly in the handle
    {
        struct RawHkdfKeyHandle
        {
            uint8_t data[64]; // Assuming max size
            uint8_t size;
        };
        const RawHkdfKeyHandle * rawKey = reinterpret_cast<const RawHkdfKeyHandle *>(&hkdfKey.KeyHandle());
        ByteSpan hkdfKeySpan(rawKey->data, rawKey->size);
        ChipLogDetail(SecureChannel, "PASE HKDF Key Material:");
        ChipLogByteSpan(SecureChannel, hkdfKeySpan);
    }
#endif
