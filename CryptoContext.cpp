                                         ...
                                         SessionInfoType infoType, SessionRole role)
{
    ...
    // Log the derived session keys for debugging
    // Note: This only works with RawKeySessionKeystore where keys are stored directly in the handle
    {
        const Crypto::Symmetric128BitsKeyByteArray * i2rKeyBytes =
            reinterpret_cast<const Crypto::Symmetric128BitsKeyByteArray *>(&i2rKey);
        const Crypto::Symmetric128BitsKeyByteArray * r2iKeyBytes =
            reinterpret_cast<const Crypto::Symmetric128BitsKeyByteArray *>(&r2iKey);

        ChipLogDetail(SecureChannel, "Session Keys Derived (Role: %s):", role == SessionRole::kInitiator ? "Initiator" : "Responder");
        ChipLogDetail(SecureChannel, "I2R Key (Initiator to Responder):");
        ChipLogByteSpan(SecureChannel, ByteSpan(*i2rKeyBytes, sizeof(Crypto::Symmetric128BitsKeyByteArray)));
        ChipLogDetail(SecureChannel, "R2I Key (Responder to Initiator):");
        ChipLogByteSpan(SecureChannel, ByteSpan(*r2iKeyBytes, sizeof(Crypto::Symmetric128BitsKeyByteArray)));
        ChipLogDetail(SecureChannel, "Attestation Challenge:");
        ChipLogByteSpan(SecureChannel, mAttestationChallenge.Span());
    }
#endif
